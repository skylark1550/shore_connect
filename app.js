...
function setMode(active) {
  ...
  // Stop live board intervals when switching away from live board mode
  if (active.btn !== liveBoardBtn) {
...
function renderResultsTable(results, fromSt, toSt) {
  ...
  results.forEach((journey) => {
    ...
    const buyButton = currentUser 
      ? `<button class="buy-ticket-btn" 
               data-journey="${escapeHtml(JSON.stringify(journey))}"
  ...
  document.querySelectorAll('.buy-ticket-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      // data-journey is HTML-entity-escaped JSON — decode it before parsing
      const rawJson = btn.dataset.journey; // browser already unescapes HTML entities from dataset
      const journey = JSON.parse(rawJson);
...
function quickTrainLookup() {
  const input = document.getElementById('adminTrainSearch');
  const resultDiv = document.getElementById('trainLookupResult');
  if (!input || !resultDiv) return;

  const trainId = normalizeName(input.value.trim());
  if (!trainId) { resultDiv.innerHTML = '<p style="color:#e74c3c;">Please enter a train ID.</p>'; return; }

  const train = timetableData?.trains.find(t => t.trainId === trainId);
  if (!train) { resultDiv.innerHTML = `<p style="color:#e74c3c;">Train "${input.value.trim()}" not found.</p>`; return; }

  const stops = stopsByTrain[trainId] || [];
  const firstStop = stops[0];
  const lastStop  = stops[stops.length - 1];
  const activeDelays = Object.values(window.trainDelays || {}).filter(d => d.trainId === trainId && d.active);
  const serviceStatus = window.serviceStatuses?.[trainId];
  ...
  let html = `
    <div style="margin-top:12px; padding:12px; background:rgba(255,255,255,0.05); border-radius:8px; border:1px solid rgba(255,255,255,0.1);">
      <strong style="font-size:1.1rem;">Train ${escapeHtml(trainId)}</strong>
      <span style="margin-left:8px; color:#a0b0c0;">${train.serviceType.replace(/_/g,' ')} • ${train.direction}</span>
      <div style="margin-top:8px;">
        <span>Origin:</span> <strong>${escapeHtml(stationLookup[firstStop?.stationId]?.name || '—')}</strong> &nbsp;→&nbsp;
        <span>Terminus:</span> <strong>${escapeHtml(stationLookup[lastStop?.stationId]?.name || '—')}</strong>
      </div>
      <div style="margin-top:4px; color:#a0b0c0; font-size:0.85rem;">
        ${stops.length} stops &nbsp;|&nbsp;
        Departs ${firstStop?.departure || '—'} &nbsp;|&nbsp;
        Arrives ${lastStop?.arrival || '—'}
      </div>`;

  if (activeDelays.length > 0) {
    const totalMin = activeDelays.reduce((s, d) => s + d.minutes, 0);
    html += `<div style="margin-top:8px; color:#f39c12;">⚠️ Delayed ${totalMin} min</div>`;
  }
  if (serviceStatus?.active && serviceStatus.status !== 'NORMAL') {
    html += `<div style="margin-top:4px; color:#e74c3c;">⛔ ${serviceStatus.status.replace(/_/g,' ')}</div>`;
  }

  html += `<div style="margin-top:12px;">
    <button class="primary" style="padding:4px 10px;font-size:0.8rem;" onclick="showFullTimetable(['${escapeHtml(trainId)}'],'${escapeHtml(firstStop?.stationId||'')}','${escapeHtml(lastStop?.stationId||'')}'); toggleAdminPanel();">
      View Full Timetable
    </button>
  </div></div>`;
